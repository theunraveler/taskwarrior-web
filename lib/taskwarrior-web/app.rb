...
class TaskwarriorWeb::App < Sinatra::Base
  ...
  get '/projects/overview/?' do
    @title = 'Projects'
    _qry = { 'status.not' => :deleted, 'project.not' => '' }
    @tasks = TaskwarriorWeb::Task.query(_qry).sort_by! do |t|
      [
        t.active? ? 0 : 1,
        -t.urgency.to_f,
        t.priority.nil?.to_s,
    ...
    end.group_by { |t| t.project.to_s }.reject do |project, tasks|
      tasks.any? { |t| t.status == 'pending' }
    end
    erb :'projects/index'
  end
  ...
  get '/projects/:name/?' do
    ...
    _qry = { 'status.not' => :deleted, :project => @title }
    @tasks = TaskwarriorWeb::Task.query(_qry).sort_by! do |t|
      [
        t.active? ? 0 : 1,
        -t.urgency.to_f,
